...
//            else if(query.value(fieldLampDirection).toInt() == 0)
...
//        }
    ...
    QSqlQuery querydirection = db->query(QString("SELECT l.Address,d.lampdirection from lampdefaultdirection d "
...
int CDevice::saveDevice(CDBSqlite *db)
{
    ...
    if((configTypeID != lampTypeID) && (configTypeID != 0))
    {
        //未知MA灯具已配置过型号
    ...
    int i = CGlobal::instance()->m_startFirepointNumber++;
    qDebug() << i;
    return 0;
}
