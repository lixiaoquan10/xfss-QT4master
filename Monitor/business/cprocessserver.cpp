...
{
    qDebug() << "CprocessServer::slot_serverlisten"
...
{
    qDebug() << "CprocessServer::slot_handleNewConnection"
             << QDateTime::currentDateTime().toString("HH:mm:ss:zzz");
    m_tcpSocket = m_tcpServer->nextPendingConnection();  // 接受新连接
    m_isMasterConnected = true;
    m_timer->stop();
    CGlobal::instance()->TimeInterval()->setComStatus();
    QString str = QString("主从客户端连接成功: %1:%2").arg(m_tcpSocket->peerAddress().toString()).arg(m_tcpSocket->peerPort());
    //发送客户端数据保存到txt文件
    CGlobal::instance()->processSocket()->saveDataToTxt("/home/xfss/root/logfile/masterData.txt", str);
...
//    //清空数据监听文本
//    CGlobal::instance()->processSocket()->clearTxtData("/home/xfss/root/logfile/masterData.txt");
}

void CprocessServer::onSocketError(QAbstractSocket::SocketError error)
{
    qDebug() << "CprocessServer::onSocketError"
             << QDateTime::currentDateTime().toString("HH:mm:ss:zzz");
    if (m_tcpSocket) {
        ...
        CGlobal::instance()->processSocket()->saveDataToTxt("/home/xfss/root/logfile/masterData.txt", dataWithTimestamp);
    }
}
...
{
    qDebug() << "CprocessServer::slot_heartBeat"
             << QDateTime::currentDateTime().toString("HH:mm:ss:zzz");
...
{
    qDebug() << "CprocessServer::slot_readClientData"
             << QDateTime::currentDateTime().toString("HH:mm:ss:zzz");
    if (m_tcpSocket) {
...
{
    qDebug() << "CprocessServer::processReceiveData"
             << QDateTime::currentDateTime().toString("HH:mm:ss:zzz");
    QString timestamp = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss:zzz");
    QString dataWithTimestamp;
...
{
    qDebug() << "CprocessServer::replyXmlMsg"
             << QDateTime::currentDateTime().toString("HH:mm:ss:zzz");
...
{
    qDebug() << "CprocessServer::slot_handleDisconnected"
             << QDateTime::currentDateTime().toString("HH:mm:ss:zzz");
    if (m_tcpSocket && m_isMasterConnected) {
        m_isMasterConnected = false;
        m_tcpSocket->deleteLater(); // 删除客户端socket对象
        QString str = QString("主从客户端断开连接: %1:%2").arg(m_tcpSocket->peerAddress().toString()).arg(m_tcpSocket->peerPort());
        //发送客户端数据保存到txt文件
        CGlobal::instance()->processSocket()->saveDataToTxt("/home/xfss/root/logfile/masterData.txt", str);
        m_timer->start(1000);
        m_heartTimer->stop();
...
{
    qDebug() << "CprocessServer::slot_controlMasterTxtFileSize"
             << QDateTime::currentDateTime().toString("HH:mm:ss:zzz");
    //设置文档大小最大为1M
    CGlobal::instance()->processSocket()->controlTxtFileSize("/home/xfss/root/logfile/masterData.txt",1024*1024);
}
