...
void CDesignSplashView::setArrowPrimitiveItem(QPixmap* background, CDevice* device)
{
    ...
    bool isEmergency;
    if(CGlobal::instance()->m_isEmergency || device->deviceValue(DEVICE_VALUE_EMERGENCY).toString() == "应急")
        isEmergency = true;
    // 绘制左箭头
...
void CDesignSplashView::setChangeExitPrimitiveItem(QPixmap* background, CDevice* device)
{
    ...
    // 限制绘制区域，只在长方形区域内绘制文字
    painter.setClipRect(rect);
//    if(device->deviceValue(DEVICE_VALUE_EMERGENCY) == "应急" || CGlobal::instance()->m_isEmergency == true)
//        painter.drawText(rect, Qt::AlignCenter, "禁止入内");  // 在长方形内居中绘制文字
//    else
