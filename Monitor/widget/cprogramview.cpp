...
void CProgramView::slot_resetControllerView()
{
    ...
    if(CGlobal::instance()->centerFrameContainer()->currentWidget() != CGlobal::instance()->loginInformation())
        return;
...
//    updateTreeThread *thread = new updateTreeThread(this);
//    connect(thread, SIGNAL(updateTreeFinished()), this, SLOT(slot_updateTreeFinished()), Qt::QueuedConnection);
//    thread->start();
}

void CProgramView::slot_updateTreeFinished()
{
    qDebug() << "CProgramView::slot_updateTreeFinished"
...
{
    qDebug() << "updateTreeThread::run"
             << QDateTime::currentDateTime().toString("HH:mm:ss:zzz");
    CGlobal::instance()->programTreeView()->resetControllerTreeView();
    emit updateTreeFinished();  // 发送成功信号
}
                            ...
                            for(int m=0; m<devices.count(); m++){
                                for(int n = m+1; n<devices.count(); n++)
                                {
                                            ...
                                            && devices.at(m)->isDeviceLogin() && devices.at(n)->isDeviceLogin())
                                    {
                                        ...
                                        messageBox.button(QMessageBox::Ok)->setText("确定");
                                        QString log;
                                        log = tr("CAN%1").arg(canport->canportAdd()-2) + tr("集中电源%2").arg(distribution->distributionAddress())
                                                + tr("回路%3").arg(loop->loopAdd()) + tr("地址%4").arg(devices.at(m)->deviceAdd()) + "存在重码";
                                        messageBox.setText(log);
                                        messageBox.exec();
                                        reflag = true;
                                        return;
